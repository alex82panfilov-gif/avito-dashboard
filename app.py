# app.py (Добавлена фильтрация итоговых строк)
...
# <<< ИЗМЕНЕНИЕ: Добавлена фильтрация строк, содержащих "Итог" >>>
def process_uploaded_file(uploaded_file):
    try:
        ...
        # --- НОВАЯ СТРОКА: Фильтруем строки, содержащие "Итог" в столбце "Вертикаль" ---
        # na=False обрабатывает возможные пустые ячейки, case=False делает поиск нечувствительным к регистру
        df = df[~df['Вертикаль'].astype(str).str.contains('Итог', na=False, case=False)]
...
if df.empty:
    ...
    df['Вертикаль'] = df['Вертикаль'].astype(str); df['Подрядчик'] = df['Подрядчик'].astype(str)
    df['Тип'] = df['Тип'].astype(str); df['Город'] = df['Город'].astype(str)
